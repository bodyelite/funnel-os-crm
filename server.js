  ...
  if(leads[idx].botActive!==false){
    ...
    if(esKeywordCalif(message)&&!leads[idx].keywordAlertSent){
      ...
      try{
        ...
        const resComp=await openai.chat.completions.create({model:'gpt-4o-mini',temperature:0.3,max_tokens:120,messages:[{role:'system',content:'Eres un asistente CRM experto. Analiza el HISTORIAL DE CHAT y las NOTAS del vendedor. Redacta una CONCLUSION ejecutiva propia de máximo 2 líneas. REGLAS: (1) NO copies ni repitas frases textuales del chat o las notas. (2) Síntesis propia: qué auto busca, cómo quiere pagar (crédito/contado/no indicado), si tiene retoma, próxima acción sugerida. (3) Tono directo para el vendedor. Solo la conclusión, sin encabezado.'},{role:'user',content:'HISTORIAL:\n'+histSnip+(notasSnip?'\nNOTAS DEL VENDEDOR:\n'+notasSnip:'')}]});
