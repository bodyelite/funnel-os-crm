...
self.addEventListener('push',e=>{
  ...
  e.waitUntil(self.registration.showNotification(data.title,{body:data.body,icon:'/icon-192.svg',badge:'/icon-192.svg',tag:'rmg-'+(data.leadId||Date.now()),renotify:true,silent:false,vibrate:[200,100,200],data:{count:data.count||1}}).then(()=>navigator.setAppBadge?navigator.setAppBadge(data.count||1).catch(()=>{}):Promise.resolve()));
});
